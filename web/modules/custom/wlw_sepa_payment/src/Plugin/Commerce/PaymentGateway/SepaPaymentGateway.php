...
namespace Drupal\wlw_sepa_payment\Plugin\Commerce\PaymentGateway;
 ...
 *   id = "wlw_sepa_payment_gateway",
 *   label = @Translation("SEPA Payment (Lastschift)"),
 *   display_label = @Translation("SEPA Payment"),
 *   forms = {
 *     "add-payment-method" = "Drupal\wlw_sepa_payment\PluginForm\SepaPaymentMethodAddForm",
 *     "receive-payment" = "Drupal\commerce_payment\PluginForm\PaymentReceiveForm",
 *   },
 *   modes = {
 *     "n/a" = @Translation("N/A"),
 *   },
 *   payment_method_types = {"sepa_payment"},
 *   requires_billing_information = FALSE,
 * )
 */
class SepaPaymentGateway extends OnsitePaymentGatewayBase {
  ...
  public function createPaymentMethod(PaymentMethodInterface $payment_method, array $payment_details) {
    $required_keys = [
      // The expected keys from Drupal\wlw_sepa_payment\PluginForm\SepaPaymentMethodAddForm;
      'sepa_payment_iban', 'sepa_payment_bic', 'sepa_payment_confirm',
    ];

    foreach ($required_keys as $required_key) {
      if (empty($payment_details[$required_key])) {
        throw new \InvalidArgumentException(sprintf('$payment_details must contain the %s.', $required_key));
      }
    }

    $payment_method->sepa_payment_iban = $payment_details['sepa_payment_iban'];
    $payment_method->sepa_payment_bic = $payment_details['sepa_payment_bic'];
    $payment_method->sepa_payment_confirm = $payment_details['sepa_payment_confirm'];
